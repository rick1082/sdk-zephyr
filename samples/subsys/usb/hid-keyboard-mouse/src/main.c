...
int main(void)
{
	...
	UDC_STATIC_BUF_DEFINE(report_mouse, MOUSE_REPORT_COUNT);
	while (true) {
		struct kb_event kb_evt;
		...
		case 114:
			LOG_INF("INPUT_KEY_1");
			report_mouse[MOUSE_X_REPORT_IDX] += 5;
			ret = hid_int_ep_write(hid_dev_mouse, report_mouse, MOUSE_REPORT_COUNT, NULL);
			if (ret) {
