...
void tlv320_setup(void)
{
    ...
    k_sleep(K_MSEC(10)); // Wait for reset to complete and internal memories to initialize (min 1ms)
    ...
    dac_i2c_write(&dev_i2c, 0x04, 0x03 | (0b11 << 0)); // Set PLL_CLKIN source to MCLK, CODEC_CLKIN source to PLL_CLK
    ...
    //     Note: Page 0 / Register 8 must be written immediately after Page 0 / Register 7 for D-value update.
    dac_i2c_write(&dev_i2c, 0x08, 0xB0); // D[7:0] for D=3760

    // (b) Power up PLL
    //     Page 0 / Register 5 (0x05): PLL P and R Values
    //     Set D7 = 1 to power up PLL, keeping P=1, R=1.
    dac_i2c_write(&dev_i2c, 0x05, (1 << 7) | (0b001 << 4) | (0b0001 << 0)); // Power up PLL, P=1, R=1
    ...
    //     Configure BCLK N-divider for 48KHz, 16-bit stereo I2S (BCLK = 2 * 16 * 48KHz = 1.536 MHz)
    //     BDIV_CLKIN = DAC_MOD_CLK = CODEC_CLKIN / (NDAC * MDAC) = 86.016 MHz / (7 * 2) = 6.144 MHz
    //     N = BDIV_CLKIN / BCLK = 6.144 MHz / 1.536 MHz = 4
    //     Page 0 / Register 30 (0x1E): BCLK N_VAL
    //     D7=1 (power up), D6-D0 = 0000100 (N=4)
    ...
    //     Other bits (D7, D6, D5, D2, D1, D0) will be configured for power up later, or left as default/recommended.
    //     For now, set common mode and ensure power bits are off.
    ...
    dac_i2c_write(&dev_i2c, 0x1F, 0xC0 | (0b00 << 3)); // HPL and HPR powered up, 1.35V common mode
    ...
    dac_i2c_write(&dev_i2c, 0x3F, 0xD4); // Powerup DAC left and right channels (soft step enabled)
...
{   
    ...
    while(1){
        ...
        NRF_I2S20->CONFIG.MCKFREQ = 0x80000000;
        printf("I2S20 MCKFREQ register: 0x%08lx\n", (unsigned long)NRF_I2S20->CONFIG.MCKFREQ);
        k_sleep(K_MSEC(2000));
        NRF_I2S20->CONFIG.MCKEN=0; // Disable MCK
        NRF_I2S20->CONFIG.MCKEN=1; // Disable MCK
        gpio_pin_set_dt(&led, 0);
        NRF_I2S20->CONFIG.MCKFREQ = 0x800f0000; // Reset MCKFREQ to 0
        printf("I2S20 MCKFREQ register: 0x%08lx\n", (unsigned long)NRF_I2S20->CONFIG.MCKFREQ);
        k_sleep(K_MSEC(2000));
        NRF_I2S20->CONFIG.MCKEN=0; // Disable MCK
        NRF_I2S20->CONFIG.MCKEN=1; // Disable MCK
        ...
        NRF_I2S20->CONFIG.MCKEN=0; // Disable MCK
        NRF_I2S20->CONFIG.MCKEN=1; // Disable MCK
        NRF_I2S20->CONFIG.MCKFREQ = 0x80000f00; // Reset MCKFREQ to 0
        printf("I2S20 MCKFREQ register: 0x%08lx\n", (unsigned long)NRF_I2S20->CONFIG.MCKFREQ);
        k_sleep(K_MSEC(2000));
    }
	return 0;
